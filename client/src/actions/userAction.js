...
import { FETCH_CURRENT_USER, SEARCH_USER, FOLLOWING_USER, FOLLOWING_USER_FROM_LIST } from './types';
...
export const followUserFromList = (userId, visitedUserId) => async dispatch => {
  try {
    ...
    if (visitedUserId) {
      dispatch({ type: FOLLOWING_USER_FROM_LIST, payload: { following: request.data.following } });
    } else {
